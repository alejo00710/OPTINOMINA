...
export default function EditableCell({ value, onChange, isOverridden, isCalculated = false, isCurrency = false, isDecimal = false, extraClasses = "" }) {
  ...
  return (
    <div
      title={isOverridden ? "Valor manual forzado (Override)" : (isCalculated ? "Dato calculado desde Liquidación" : "")}
      className={`relative group px-3 py-2 -mx-3 -my-2 rounded-lg transition-all duration-200 cursor-text ${getContainerClasses()}`}
      onClick={() => setIsEditing(true)}
    >
      {isOverridden && (
        <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-amber-400" title="Valor manual forzado (Override)"></span>
      )}
      {isEditing ? (
        <input
          type="text"
